...
写博客文章看起来简单，但每次都有七八个细节容易遗漏：modDatetime 更新了吗？标签和已有的冲突吗？更新记录加了吗？
...
2. 填充 11 个 frontmatter 字段（对齐 Zod schema），时间取当前 UTC
...
**规范写死，不靠记忆**。my-blog-build 把 frontmatter 的每个字段、标签的命名规则、更新记录的格式都写成模板。AI 不需要"理解"规范，只需要"遵循"规范——这比口头交代可靠得多，也避免了"这次记得说，下次忘了"的问题。
...
写博客文章时，不再需要交代 frontmatter 格式、标签命名、更新记录的写法。AI 自动按 my-blog-build 的检查清单逐项执行，文末追加带时间戳的更新记录。我只需要确认内容，然后说"推送"。
...
<details>
<summary>📝 更新记录（2026-07-07 17:35:00）</summary>
